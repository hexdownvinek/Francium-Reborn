...
public class GlyphPageFontRenderer {
    ...
    public int drawStringWithShadow(PoseStack poses, String text, float x, float y, int color) {
        return drawString(poses, text, x, y, color, true);
    }

    public int drawStringWithShadow(PoseStack poses, String text, double x, double y, int color) {
        return drawString(poses, text, (float) x, (float) y, color, true);
    }

    public int drawString(PoseStack poses, String text, float x, float y, int color) {
        return drawString(poses, text, x, y, color, false);
    }

    public int drawString(PoseStack poses, String text, double x, double y, int color) {
        return drawString(poses, text, (float) x, (float) y, color, false);
    }

    public int drawCenteredString(PoseStack poses, String text, double x, double y, float scale, int color) {
        return drawString(poses, text, (float) x - getStringWidth(text) / 2, (float) y - getFontHeight(), scale, color, false);
    }

    public int drawCenteredString(PoseStack poses, String text, double x, double y, int color) {
        return drawString(poses, text, (float) x - getStringWidth(text) / 2, (float) y - (getFontHeight() / 2), color, false);
    }

    public int drawCenteredStringWithShadow(PoseStack poses, String text, double x, double y, int color) {
        return drawString(poses, text, (float) x - getStringWidth(text) / 2, (float) y - (getFontHeight() / 2), color, true);
    }

    public int drawString(PoseStack poses, String text, float x, float y, float scale, int color, boolean dropShadow) {
        this.resetStyles();
        int i;

        if (dropShadow) {
            i = this.renderString(poses, text, x + 1.0F, y + 1.0F, scale, color, true);
            i = Math.max(i, this.renderString(poses, text, x, y, scale, color, false));
        } else {
            i = this.renderString(poses, text, x, y, scale, color, false);
        }
     ...
     */
    public int drawString(PoseStack poses, String text, float x, float y, int color, boolean dropShadow) {
        this.resetStyles();
        int i;

        if (dropShadow) {
            i = this.renderString(poses, text, x + 1.0F, y + 1.0F, color, true);
            i = Math.max(i, this.renderString(poses, text, x, y, color, false));
        } else {
            i = this.renderString(poses, text, x, y, color, false);
        }
     ...
     */
    private int renderString(PoseStack poses, String text, float x, float y, int color, boolean dropShadow) {
        if (text == null) {
            return 0;
        } else {
            ...
            this.posX = x * 2.0f;
            this.posY = y * 2.0f;
            this.renderStringAtPos(poses, text, dropShadow, color);
            return (int) (this.posX / 4.0f);
        }
    }

    private int renderString(PoseStack poses, String text, float x, float y, float scale, int color, boolean dropShadow) {
        if (text == null) {
            return 0;
        } else {
            ...
            this.posX = x * 2.0f;
            this.posY = y * 2.0f;
            this.renderStringAtPos(poses, text, scale, dropShadow, color);
            return (int) (this.posX / 4.0f);
        }
    }
     ...
     */
    private void renderStringAtPos(PoseStack poses, String text, boolean shadow, int color) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (c0 == 167 && i + 1 < text.length()) {
                ...
                float f = glyphPage.drawChar(poses, c0, posX, posY, g, k, h, alpha);
    ...
    private void renderStringAtPos(PoseStack poses, String text, float scale, boolean shadow, int color) {
        GlyphPage glyphPage = getCurrentGlyphPage();
        float alpha = (float) (color >> 24 & 255) / 255.0F;
        float g = (float) (color >> 16 & 255) / 255.0F;
        float h = (float) (color >> 8 & 255) / 255.0F;
